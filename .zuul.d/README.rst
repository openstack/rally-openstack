...
.. warning:: A task job resolves ``rally_task`` against ``rally_task_src_dir``,
    which defaults to the checkout of the project that triggers it. Overriding
    it makes it possible to reuse a task file shipped in this repo from another
    project (e.g. rally). This is technically supported but not recommended in
    general, as it couples that project to rally-openstack's on-disk layout.
